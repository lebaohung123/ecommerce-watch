...
const TopSeller = () => {
	...
	const handleFollowClick = (index: number) => {
		setAuthors(prevAuthors => prevAuthors.map((author, i) => (i === index ? { ...author, isFollowing: !author.isFollowing } : author)));
	}
			...
			<ul>
				{authors.map((author, index) => (
					<li key={index} className="flex items-center justify-between mb-4">
						...
						<button onClick={() => handleFollowClick(index)} className={`py-1 px-3 rounded ${author.isFollowing ? "bg-red-500 text-white" : "bg-black text-white"}`}>
